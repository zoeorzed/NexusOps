...
- Redis 保存当前会话工作记忆
- 本地持久化保存历史会话摘要和用户画像
- `/chat` 生成只采用当前会话的消息与摘要，避免其他会话的订单和退款意愿混入当前请求
- 跨会话摘要与用户画像仍保存，当前业务回答不自动注入这些信息；恢复跨会话作答需先设计用户确认与事实归属机制
- 异步更新用户画像，避免阻塞主响应链路
...
[业务场景验证](docs/business-validation.md) 通过真实本机 HTTP 检查复合问题、退款阶段、工作记忆边界和无答案问题，并用冻结参考文档核对检索结果。结构检查、检索命中与回答内容复核分别记录，不以 HTTP 200 或模型自评作为业务正确率。

[并发负载验证](docs/load-testing.md) 使用实际编排器、Agent 和有界线程池，只将模型网关替换为可控延迟/失败实现。运行 `./scripts/run-load-test.ps1`，可复现正常负载、队列饱和、超时、部分失败及同池恢复，并生成逐请求结果、延迟分位数、成功吞吐与失败原因统计。它不经过 HTTP、意图识别、检索或 Redis，不代表真实模型吞吐或整个 `/chat` 的延迟。CI 同样执行该负载入口并上传结果。

2026-09-20 实测：71 项 Java 测试与 25 项 Python 测试通过；12 条真实模型业务请求的 137 项结构检查通过，8 条有答案检索均首位命中；受控并发完成 172 个请求和 15 项行为检查。业务验证发现并修复了复合路由漏项与跨会话订单混用，内容复核仍有资料外推断、材料清单重复和表达问题。逐条证据、并发配置和结论边界见 [业务与并发验证报告](docs/verification-business-load-2026-09-20.md)。

2026-09-21 回答质量收尾：修正资料外政策推断、已知信息重复收集、领域日期误归及重复扣款与普通退款规则混用，简化用户可见措辞。76 项 Java 测试、原有 25 项辅助 Python 测试和 172 请求受控并发回归通过；原 12 条真实业务场景的 137 项结构检查通过，并保留三轮逐条内容复核及非阻断表达问题。详见 [回答质量记录](docs/answer-quality-2026-09-21.md)。
